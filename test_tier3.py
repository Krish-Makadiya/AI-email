...
def run_tier3_test():
    """
    Simulates a high-urgency incident to verify Tier 3 Escalation logic.
    Constraint: Should result in 70-85% urgency and 'flagged_high' status.
    """
    dynamic_id = get_random_card_id()
    
    payload = {
        "sender_email": "cto@corporate.com",
        "subject": "CRITICAL INCIDENT: Database Connection Pool Exhaustion",
        "content": "Production database is dropping connections. API is returning 500s for 40% of traffic. WE NEED THIS FIXED NOW.",
        "is_1on1": True,
        "received_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "card_id": dynamic_id
    }

    print(f"[QA] Dispatching Tier 3 Test Signal to Gateway...")
    ...
    try:
        ...
        if 70 <= score < 85:
            print(f"SUCCESS: Signal correctly categorized as Tier 3 (High Urgency).")
        else:
            print(f"NOTE: Signal scored {score}%. Target was 70-85%.")
